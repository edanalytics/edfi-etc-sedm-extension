...
COMMENT ON COLUMN sedm.IEPServiceDeliveryExternalServiceProvider.IEPFinalizedDate IS 'The date the most recent IEP was finalized.  Note: Date interpretation may vary. Ed-Fi recommends inclusive dates, but states may define dates as inclusive or exclusive. For calculations, align with local guidelines.';
...
COMMENT ON COLUMN sedm.IEPServiceDeliveryExternalServiceProvider.IEPServicingEducationOrganizationId IS 'The identifier assigned to an education organization.';
...
COMMENT ON COLUMN sedm.IEPServiceDeliveryExternalServiceProvider.StudentIEPAssociationID IS 'A unique identifier assigned by the provider or source system of IEP services.';
