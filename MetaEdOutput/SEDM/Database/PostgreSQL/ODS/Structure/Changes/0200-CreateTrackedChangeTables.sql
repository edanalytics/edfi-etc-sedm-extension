...
(
       ...
       discriminator varchar(128) NULL,
       createdate timestamp NOT NULL DEFAULT (now()),
       CONSTRAINT iepgoal_pk PRIMARY KEY (ChangeVersion)
);
END IF;
...
(
       ...
       discriminator varchar(128) NULL,
       createdate timestamp NOT NULL DEFAULT (now()),
       CONSTRAINT iepservicedelivery_pk PRIMARY KEY (ChangeVersion)
);
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'tracked_changes_sedm' AND table_name = 'studentiepassociation') THEN
...
(
       ...
       discriminator varchar(128) NULL,
       createdate timestamp NOT NULL DEFAULT (now()),
       CONSTRAINT studentiepassociation_pk PRIMARY KEY (ChangeVersion)
);
END IF;
